...
def main(args):
    ...
    cur_save_file = os.path.join(args.exp_path, f"attack_{args.num_split}_{args.split_idx}.json")
    if os.path.exists(cur_save_file):
        full_exp_record = json.load(open(cur_save_file, encoding="utf-8"))
    ...
    for index_prompt, target_prompt in enumerate(target_prompts):
        ...
        if index_prompt % args.num_split != args.split_idx:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-split", type=int, required=True)
